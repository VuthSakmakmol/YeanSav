...
<div class="container text-center my-5" style="background-color: #f0a500; color: #333; padding: 50px 0;">
    ...
    <div class="row justify-content-center mt-5" style="gap: 10px;">
        @foreach($services as $index => $service)
            <div class="col-md-4 col-12 mb-5 service-item" style="margin: 10px;">
                <div class="card h-100" style="border: none; position: relative; overflow: hidden; border-radius: 10px;">
                    ...
                    <div class="card-body text-start p-4" style="background: rgba(0, 0, 0, 0.6); color: white; position: absolute; bottom: 0; width: 100%;">
                        ...
                        @if(auth()->check() && auth()->user()->is_admin)
                            <a href="{{ route('service.detail.create', $service->id) }}" class="btn btn-primary">Add Detail</a>
                            <a href="{{ route('service.details.show-all', $service->id) }}" class="btn btn-sm btn-info">View Details</a>
                        @else
                            @foreach ($service->serviceDetails as $serviceDetail)
                                <a href="{{ route('service.detail.show', [$service->id, $serviceDetail->id]) }}" class="btn btn-outline-light">See Detail</a>
                            @endforeach
                        @endif 
                    </div>
